...
class RegistrationTest extends WebTestCase
{
    ...
    public function testUserShouldNOTBeRegisteredPassword()
    {
        ...
        $form = $crawler->selectButton('sign up')->form([
            'registration_form[email]' => 'test@test.com',
            'registration_form[plainPassword]' => 'hi',
            'registration_form[agreeTerms]' => false
        ]);
        $client->submit($form);
        // Check that the response is a redirect to the login page
    ...
    public function testUserShouldNOTBeRegisteredEmail()
    {
        //user should NOT be registered if the email is malformed
        $client = static::createClient();
        // Submit the registration form with valid data
        $crawler = $client->request('GET', '/register');
        $form = $crawler->selectButton('sign up')->form([
            'registration_form[email]' => 'testtest.com',
            'registration_form[plainPassword]' => 'hi',
            'registration_form[agreeTerms]' => false
        ]);
        $client->submit($form);
