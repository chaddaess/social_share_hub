...
class FacebookCallbackController extends AbstractController
{
    ...
    public function index(UserRepository $userDb, EntityManagerInterface $manager, Request $request): Response
    {
        ...
        try {
            ...
            if ($user_exist) {
                $user_exist->setFacebookId($user->getId());
                $manager->persist($user_exist);
                $manager->flush();
            }
            $accessToken = "1051784289526895|FT6WCT81AO0p9Z1Lmg7xnbPoO28";
            $userPicture = "http://graph.facebook.com/$id/picture?type=large&access_token=$accessToken";
